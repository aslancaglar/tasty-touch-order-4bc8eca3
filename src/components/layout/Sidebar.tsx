...
import { LayoutDashboard, Store, ChevronLeft, ChevronRight, LogOut } from "lucide-react";
...
export function Sidebar({
  ...
  const {
    t
  } = useTranslation(forceDefaultLanguage ? 'en' : language);
      ...
      if (match && match[1]) {
        ...
        try {
          const {
            data,
            error
          } = await supabase.from('restaurants').select('ui_language').eq('id', restaurantId).single();
  ...
  if (isLoading) {
    return <div className="flex flex-col h-screen bg-white border-r border-gray-200 w-16">
        <div className="flex-1 py-4 flex items-center justify-center">
          <div className="h-8 w-8 animate-spin rounded-full border-2 border-primary border-t-transparent"></div>
        </div>
      </div>;
  }
  return <div className={cn("flex flex-col h-screen bg-white border-r border-gray-200 transition-all duration-300", collapsed ? "w-16" : "w-64")}>
      <div className="flex items-center justify-between p-4 border-b">
        {!collapsed && <h1 className="text-xl font-bold text-kiosk-primary">Qimbo - Kiosk</h1>}
        <button onClick={() => setCollapsed(!collapsed)} className="p-1.5 rounded-lg text-gray-500 hover:bg-gray-100">
      ...
      <nav className="flex-1 py-4 overflow-y-auto">
        <ul className="space-y-1 px-2">
          {isAdmin && adminSidebarItems.map(item => <li key={item.href}>
              <Link to={item.href} className={cn("flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition-colors", location.pathname === item.href ? "bg-primary/10 text-primary" : "text-gray-600 hover:text-primary hover:bg-primary/5")}>
      ...
      <div className="p-4 border-t border-gray-200">
        {!collapsed ? <div className="flex flex-col space-y-4">
            ...
            <button onClick={handleSignOut} className="flex items-center px-3 py-2 text-sm text-gray-600 hover:text-primary hover:bg-primary/5 rounded-lg">
              <LogOut size={18} className="mr-2" />
              {t("sidebar.signOut")}
            </button>
          </div> : <button onClick={handleSignOut} className="w-full flex justify-center p-2 text-gray-600 hover:text-primary hover:bg-primary/5 rounded-lg" title={t("sidebar.signOut")}>
            <LogOut size={20} />
          </button>}
      </div>
    </div>;
