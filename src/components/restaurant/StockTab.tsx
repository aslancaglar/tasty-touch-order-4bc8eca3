...
const StockTab = ({
  ...
  const handleMenuItemStockToggle = async (item: MenuItem) => {
    try {
      ...
      setMenuItems(prev => ({
        ...prev,
        [item.category_id]: prev[item.category_id].map(menuItem => menuItem.id === item.id ? updatedItem : menuItem)
      }));
      toast({
        title: "Success",
  ...
  const handleToppingStockToggle = async (topping: Topping) => {
    try {
      ...
      setToppings(prev => ({
        ...prev,
        [topping.category_id]: prev[topping.category_id].map(t => t.id === topping.id ? updatedTopping : t)
      }));
      toast({
        title: "Success",
  ...
  return <Tabs defaultValue="menu-items">
      ...
      <TabsContent value="menu-items">
        <div className="space-y-6">
          {categories.map(category => <Card key={category.id} className="p-6">
              <h3 className="text-lg font-semibold mb-4">{category.name}</h3>
              <div className="space-y-4">
                {menuItems[category.id]?.map(item => <div key={item.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex items-center space-x-4">
                      {item.image && <img src={item.image} alt={item.name} className="h-12 w-12 object-cover rounded-md" />}
                      <div>
                    ...
                    <div className="flex items-center space-x-2">
                      {item.in_stock ? <Package className="h-4 w-4 text-green-500" /> : <PackageOpen className="h-4 w-4 text-red-500" />}
                      <Switch checked={item.in_stock} onCheckedChange={() => handleMenuItemStockToggle(item)} />
                    </div>
                  </div>)}
              </div>
            </Card>)}
      ...
      <TabsContent value="toppings">
        <div className="space-y-6">
          {toppingCategories.map(category => <Card key={category.id} className="p-6">
              <h3 className="text-lg font-semibold mb-4">{category.name}</h3>
              <div className="space-y-4">
                {toppings[category.id]?.map(topping => <div key={topping.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div>
                    ...
                    <div className="flex items-center space-x-2">
                      {topping.in_stock ? <Package className="h-4 w-4 text-green-500" /> : <PackageOpen className="h-4 w-4 text-red-500" />}
                      <Switch checked={topping.in_stock} onCheckedChange={() => handleToppingStockToggle(topping)} />
                    </div>
                  </div>)}
              </div>
            </Card>)}
