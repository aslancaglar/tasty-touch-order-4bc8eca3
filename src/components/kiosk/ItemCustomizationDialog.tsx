  ...
  return <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <DialogContent className="w-[95vw] max-w-[95vw] md:w-[85vw] md:max-w-[85vw] max-h-[95vh] p-3 md:p-4 flex flex-col">
        ...
        <div className="space-y-4 flex-1 overflow-y-auto pr-2 py-2">
          {item.options && item.options.map(option => <div key={option.id} className="space-y-1.5">
          ...
          {item.toppingCategories && item.toppingCategories.filter(category => shouldShowToppingCategory(category)).map(category => <div key={category.id} className="space-y-2">
              ...
              return <div key={topping.id} onClick={() => onToggleTopping(category.id, topping.id)} className="flex items-center justify-between border rounded-md p-2 hover:border-gray-300 cursor-pointer">
                        <span className={`${isSelected ? 'text-green-700 font-medium' : ''} text-sm truncate`}>
                          {topping.name}
                        </span>
                        <div className="flex items-center gap-1 shrink-0">
                          {topping.price > 0 && <span className="text-xs mr-1">
                              +{parseFloat(topping.price.toString()).toFixed(2)} {currencySymbol}
                            </span>}
                          <Button variant="outline" size="icon" onClick={e => {
                    e.stopPropagation();
                    onToggleTopping(category.id, topping.id);
                  }} className={`text-4xl p-1 h-7 w-7 rounded-full text-slate-50 font-bold ${isSelected ? 'bg-green-700 hover:bg-green-600' : 'bg-violet-800 hover:bg-violet-700'}`}>
          ...
          <div className="flex items-center justify-between py-2">
            ...
            <div className="flex items-center space-x-3">
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => quantity > 1 && onQuantityChange(quantity - 1)}>
        ...
        <DialogFooter className="mt-3">
          <div className="w-full">
            <Button onClick={onAddToCart} className="w-full bg-kiosk-primary text-xl py-6">
