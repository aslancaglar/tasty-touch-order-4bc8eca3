...
const Option = memo(({
  ...
  const memoizedChoices = useMemo(() => {
    return option.choices.map(choice => {
      ...
      const choicePrice = choice.price && choice.price > 0 
      ...
      return (
        ...
        >
          ...
          {choicePrice && <span>{choicePrice}</span>}
        </div>
      );
    });
  }, [option.choices, selectedOption, onToggleChoice, currencySymbol, option.id, option.multiple]);
...
const ToppingCategory = memo(({
  ...
  const memoizedToppings = useMemo(() => {
    return sortedToppings.map(topping => {
      ...
      const hasPrice = topping.price > 0;
  ...
  }, [sortedToppings, selectedCategory, onToggleTopping, currencySymbol, category.id]);

  return (
    <div className={`space-y-2 p-4 rounded-xl mb-4 ${bgColorClass}`}>
...
const ItemCustomizationDialog: React.FC<ItemCustomizationDialogProps> = ({
  ...
  // Pre-filter visible topping categories for faster rendering
  const visibleToppingCategories = useMemo(() => {
    return sortedToppingCategories.filter(category => shouldShowToppingCategory(category));
  }, [sortedToppingCategories, shouldShowToppingCategory]);
    ...
    <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <DialogContent className="w-[85vw] max-w-[85vw] max-h-[80vh] p-4 flex flex-col select-none">
        <DialogHeader className="pb-2">
          ...
          {item.description && (
            <DialogDescription className="text-xl text-gray-800">
              {item.description}
            </DialogDescription>
          )}
        </DialogHeader>
        ...
        <DialogFooter className="mt-3 pt-2">
          <div className="w-full flex items-center">
            <div className="flex items-center mr-4">
              ...
              <span className="font-medium text-2xl min-w-[40px] text-center">
                {quantity}
              </span>
              <Button 
                className="h-12 w-12 text-3xl flex items-center justify-center rounded-full bg-violet-800 hover:bg-violet-700 text-white" 
            ...
            >
              {t("addToCart")} - {calculatedPrice} {currencySymbol}
            </Button>
          </div>
        </DialogFooter>
      </DialogContent>
